...
# ── Cấu hình ─────────────────────────────────────────────────
CHARACTER_KEY = sys.argv[1] if len(sys.argv) > 1 else "kael"
USER_NAME     = "Minh"
TEMPERATURE   = 0.85

# Challenge text adapts per character
CHALLENGE_TEXTS = {
    "kael": "Nếu tôi nói tôi không tin thám tử thì sao?",
    "seraphine": "Nếu tôi nói tôi không tin vào sách thì sao?",
    "ren": "Nếu tôi nói âm nhạc chỉ là tiếng ồn thì sao?",
    "linh_dan": "Nếu tôi nói cocktail chỉ là nước pha màu thì sao?",
}
...
TURNS = [
    ...
    ("B", CHALLENGE_TEXTS.get(CHARACTER_KEY, "Nếu tôi nói tôi không tin bạn thì sao?")),
    ...
    # Group F — User *action* (physical interaction)
    ("F", "*xoa đầu bạn nhẹ nhàng*"),
    ("F", "*ôm bạn từ phía sau, không nói gì*"),
    ("F", "*nắm tay bạn và kéo đi*"),
]
...
GROUP_LABELS = {
    ...
    "F": "GROUP F — User *action* (physical interaction)",
}
...
def main():
    ...
    lines = []
    lines.append(SEP)
    lines.append(f"BATCH TEST — {char_name} V4.2")
    ...
    for turn_idx, (group, user_msg) in enumerate(TURNS, start=1):
        ...
        print(f"[Turn {turn_idx}] [{char_name.upper()}]:")
        response = stream_and_collect(messages, TEMPERATURE)

        conv.add_assistant(response)

        lines.append(f"[Turn {turn_idx}] [{char_name.upper()}]")
    ...
    filename  = f"batch_test_{CHARACTER_KEY}_v42_{timestamp}.txt"
